...
const { ensureClonedDataFor } = require('../services/demoData.service');

/**
 * Generate JWT tokens
 */
const generateTokens = (userId, role, varSeed) => {
...
router.post('/register-student', [
  ...
  try {
    ...
    const existingUsers = await query('SELECT id FROM users WHERE registration_number = $1 OR email = $2', [registration_number, email]
    );
    ...
    // Insert user (PostgreSQL returns the row with RETURNING)
    const result = await query(`INSERT INTO users (name, registration_number, email, password, phone_number, 
       department, year, section, role) VALUES ($1, $2, $3, $4, $5, $6, $7, $8, 'student') RETURNING id`, [name, registration_number, email, hashedPassword, phone_number, department, year, section]
    );
    ...
    // Generate tokens
  const varSeed = Math.floor(Math.random() * 1e9);
  const { accessToken, refreshToken } = generateTokens(userId, 'student', varSeed);
...
router.post('/register-teacher', [
  ...
  try {
    ...
    // Insert user (PostgreSQL returns the row with RETURNING)
    const result = await query(`INSERT INTO users (name, registration_number, email, password, phone_number, 
       department, subjects_taught, role) VALUES ($1, $2, $3, $4, $5, $6, $7, 'teacher') RETURNING id`, [name, registration_number, email, hashedPassword, phone_number, department, subjects_taught]
    );
    ...
    // Generate tokens
  const varSeed = Math.floor(Math.random() * 1e9);
  const { accessToken, refreshToken } = generateTokens(userId, 'teacher', varSeed);
    await storeRefreshToken(userId, refreshToken);
...
router.post('/login', [
  ...
  try {
    ...
    let users;
    try {
      users = await query(`SELECT id, name, registration_number, email, password, role, department, 
         year, section, subjects_taught, profile_picture, is_active 
         FROM users WHERE registration_number = $1`, [registration_number]
      );
    } catch (dbError) {
    ...
    // Ensure demo baseline is cloned for new/empty users
    try {
      await ensureClonedDataFor(user.id, user.role);
    } catch (e) {
      console.warn('Demo clone skipped:', e.message);
    }

    // Generate tokens with a fresh variation seed per login
    const varSeed = Math.floor(Math.random() * 1e9);
    const { accessToken, refreshToken } = generateTokens(user.id, user.role, varSeed);
...
router.post('/refresh', async (req, res, next) => {
  try {
    ...
    const tokens = await query('SELECT * FROM refresh_tokens WHERE token = $1 AND user_id = $2 AND expires_at > NOW()', [refreshToken, decoded.userId]
    );
...
router.get('/me', authMiddleware, async (req, res, next) => {
  try {
    const users = await query(`SELECT id, name, registration_number, email, 
