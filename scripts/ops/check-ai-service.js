 ...
 * 
 * Usage: node scripts/ops/check-ai-service.js
 */
...
const { getSecureKeyPreview, isValidOpenRouterKey, isValidGeminiKey } = require(path.join(repoRoot, 'server/utils/security.util'));
...
const envExists = fs.existsSync(path.join(repoRoot, '.env'));
if (envExists) {
...
if (configurationScore >= 90) {
    ...
    console.log('3. Verify with: node scripts/ops/verify-ai-service.js');
    ...
    console.log('- Test the service: node scripts/ops/verify-ai-service.js');
